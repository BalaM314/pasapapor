...
export function getSelectedLevel():Level | null {
	const value = Array.from(levelSelectDiv.children).filter(
...
export function getSelectedSource(def:PaporProviderName | null = null):PaporProviderName | null {
	...
	console.log(value);
	if((Object.keys(providers)).includes(value)) return value;
	else return def;
}
...
export function getURL(input:Openable){
	if(input instanceof Papor) return input.url(getSelectedSource("gceguide"));
			...
			try {
				...
				if(pasapaporInput.value.includes("amogus")) {
					...
					default:
						...
						if(papors.length == 1) window.open(getURL(papors[0]), "_blank");
						else {
				...
				outputBox.innerText = `❗ ${(err as Error).message}`;
			}
		}
	});
